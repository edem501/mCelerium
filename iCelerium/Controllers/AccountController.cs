...
namespace iCelerium.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public JsonResult LoadContituencyByRegion(int RegionID)
        {
            List<SelectListItem> lstConti = new List<SelectListItem>();
            foreach (Constituency cty in db.Constituencies.Where(c => c.District.RegionID == RegionID).ToList())
            {
                lstConti.Add(new SelectListItem { Text = cty.ConstituencyName, Value = cty.ID.ToString() });
            }
            return Json(lstConti, JsonRequestBehavior.AllowGet);
        }



        public JsonResult LoadPollingByConstituency(int ConstituencyID)
        {
            List<SelectListItem> lstPolling = new List<SelectListItem>();
            foreach (PollingStation cty in db.PollingStations.Where(c => c.ElectoralArea.Constituency1.ID == ConstituencyID).ToList())
            {
        ...
        public async Task<ActionResult> Edit(string id)
        {
            ...
            {
                itemRegions.Add(new SelectListItem { Text = rg.RegionName, Value = rg.ID.ToString() });
            }
            ...
            if (agent == null)
            {
                agent = new Agent { IsEnable = false, UserID = "", ContituencyID = 0, DistrictID = 0, RegionID = 0 };
            }
        ...
        public async Task<ActionResult> Roles()
        {
            ...
            if (cmx.Count() == 0)
            {
               var roleManage = new RoleManager<IdentityRole>(new RoleStore<IdentityRole>());
               roleManage.Create(new IdentityRole("User"));
               roleManage.Create(new IdentityRole("Admin"));
               roleManage.Create(new IdentityRole("Super Admin"));
            }
            List<SelectListItem> items = new List<SelectListItem>();
            foreach (AspNetRole com in cmx)
            {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (this.ModelState.IsValid)
            {
                ...
                {
                    this.ModelState.AddModelError("", "User ou mot de passe incorect.");
                }
            }
